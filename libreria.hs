...
quienEscribio obraConsultar =
  map autor (filter (\libro -> obra libro == obraConsultar) libros)

-- ¿Qué obra/s escribió cierta persona?
queEscribio :: String -> [String]
queEscribio autorConsultar =
  map obra (filter (\libro -> autor libro == autorConsultar) libros)
...
autores =
  ...
  , Autor
      "isaacAsimov"
      ["fundacion", "yoRobot", "elFinDeLaEternidad", "laBusquedaDeLosElementos"]
...
quienesEscribieron obraBuscada =
  map nombre (filter (escribioObra obraBuscada) autores)
...
escribioUnaObra autorBuscado =
  any
    (\autor -> nombre autor == autorBuscado && length (obras autor) > 0)
    autores
